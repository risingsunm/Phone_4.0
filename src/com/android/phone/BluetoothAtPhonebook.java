...
public class BluetoothAtPhonebook {
    ...
    private static final int INVALID_SUB = 99;
    ...
    public PhoneNumberSub getLastDialledNumber() {
        String[] projection = {Calls.NUMBER, Calls.SUBSCRIPTION};
        ...
        cursor.moveToNext();
        PhoneNumberSub phoneNumSub = new PhoneNumberSub();
        int column = cursor.getColumnIndexOrThrow(Calls.NUMBER);
        phoneNumSub.number  = cursor.getString(column);
        try {
            phoneNumSub.sub = cursor.getInt(cursor.getColumnIndexOrThrow(Calls.SUBSCRIPTION));
        } catch (Exception e) {
            Log.e(TAG,"get subscription failed! "+e.toString());
            phoneNumSub.sub = INVALID_SUB;
        }
        if (DBG)
            Log.d(TAG,"number = "+phoneNumSub.number+" sub="+phoneNumSub.sub);
        cursor.close();
        return phoneNumSub;
    }
