    ...
    public class OutgoingCallReceiver extends BroadcastReceiver {
        ...
        public void doReceive(Context context, Intent intent) {
            ...
            if (mIPCall) {
                ...
                if(IPPrefix==null||IPPrefix.equals("")){
                    if(mSubscription==0){
                        IPPrefix = "17909";
                        Settings.System.putString(getContentResolver(),
                                Settings.System.IPCALL_PREFIX[mSubscription],IPPrefix);
                    }else if(mSubscription==1){
                        String getSim2Operator = ((MSimTelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE)).getSimOperator(1);
                        if (getSim2Operator != null){
                            ...
                            // China Mobile
                                ...
                                Log.d(TAG,"not China Mobile or China Unicom");
                            }
                        }else{
                            IPPrefix = "17951";
                        }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (!isEccNum && TelephonyManager.getDefault().isMultiSimEnabled() && /**promptEnabled &&**/
               (activeSubCount() > 1) && (!isIntentFromBluetooth(intent)) &&
                       ...
                       (!isSIPCall(number, intent))&&(!intent.getBooleanExtra("directDial", false))) {
            ...
            if (TelephonyManager.getDefault().isMultiSimEnabled() && activeSubCount() > 1
                ...
                /*Begin: Modified by siliangqi for 112 2012-8-3*/
                //mSubscription = PhoneApp.getInstance().getVoiceSubscription();
                mSubscription = intent.getIntExtra(MSimConstants.SUBSCRIPTION_KEY, -1);
                if(mSubscription==-1){
                    if(!TelephonyManager.getTelephonyProperty("gsm.sim.state",0,"").equals("ABSENT")){
                        mSubscription = 0;
                    }else if(!TelephonyManager.getTelephonyProperty("gsm.sim.state",1,"").equals("ABSENT")){
                        mSubscription = 1;
                    }
                }
                /*End: Modified by siliangqi for 112 2012-8-3*/
