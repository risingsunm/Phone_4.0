...
/*Begin: Modified by siliangqi for location_search 2012-4-26*/
import android.content.ContentValues;
import android.net.Uri;
import android.database.Cursor;
import android.provider.Settings.SettingNotFoundException;
import android.provider.Settings;
/*End: Modified by siliangqi for location_search 2012-4-26*/
...
public class CallNotifier extends Handler
    ...
    /* Begin: Modified by zxiaona for connection_vibrator 2012/07/21 */
    private Call.State mPreviousCallState;
    /* End: Modified by zxiaona for connection_vibrator 2012/07/21 */
    ...
    protected CallNotifier(PhoneApp app, Phone phone, Ringer ringer,
        ...
        mAudioManager = (AudioManager) mApplication.mContext.getSystemService(Context.AUDIO_SERVICE);
                ...
                case 0:
                    switch (suppSvcNotification.code) {
                        case SuppServiceNotification.MO_CODE_UNCONDITIONAL_CF_ACTIVE :
                            ...
                            callForwardTxt = mApplication.mContext.getString(
                ...
                case 1:
                    switch (suppSvcNotification.code) {
                        ...
                        case SuppServiceNotification.MT_CODE_CALL_ON_HOLD:
                            ...
                            callForwardTxt = mApplication.mContext.getString
                                    (R.string.card_title_callonhold);
                            break;
    ...
    private void onNewRingingConnection(AsyncResult r) {
        ...
        // set phone mode to RINGING only when there is no active call
    ...
    private void registerForNotifications() {
        ...
        /*Begin: Modified by zxiaona for cdma time 2012/07/30*/
        mCM.getPhoneInCall().registerForLineControlInfo(this, PHONE_LINE_CONTROL_INFO, null);
        /*End: Modified by zxiaona for cdma time 2012/07/30*/
    ...
    public void onQueryComplete(int token, Object cookie, CallerInfo ci) {
        if (cookie instanceof Long) {
            ...
            if (isQueryExecutionTimeOK) {
                ...
                {
                    String strPath = ci.contactRingtoneUri.toString().substring(7);
                    ...
                    {
                        ci.contactRingtoneUri = null;
                        log("custom ringtone not found, set default");
                    }
                }
                else
                {
                    if (DBG) log("ci.contactRingtoneUri is null");
                }
                /*End: Modified by sunrise for RingTone 2012/08/21*/
    ...
    private void onDisconnect(AsyncResult r) {
        ...
        if (mCM.getState() == Phone.State.IDLE) {
            ...
            if (call.getState() == Call.State.HOLDING) {
                try {
                    ...
                    Log.d(LOG_TAG, "active hold call error:" + e);
                }
            }
        }
        ...
        if (c != null) {
            ...
            final long duration;
            if(CallTime.resetTime>0){
            duration = CallTime.resetTime * 1000;}
            else{duration = c.getDurationMillis();}
                ...
                if (okToLogThisCall) {
                    CallLogAsync.AddCallArgs args =
                            new CallLogAsync.AddCallArgs(
                                ...
                                /*Begin: Modified by siliangqi for C,G_Dial 2012-5-26*/
                                //callLogType, date, duration, phone.getSubscription());
                                callLogType, date, duration, phone.getPhoneType()-1);
                                /*End: Modified by siliangqi for C,G_Dial 2012-5-26*/
                    /*
                     * Begin: Modified by siliangqi for location_search
                     * 2012-4-26
                     */
                    new Thread() {
                        ...
                        public void run() {
                            int _id, myId;
                            try {
                                myId = Settings.System.getInt(mApplication.mContext.getContentResolver(),
                                        "location_id");
                            } catch (SettingNotFoundException e) {
                                myId = 0;
                            }
                            Cursor myCursor = null;
                            /*Begin: Modified by bxinchun 2012-08-02, add try block to fix bug.*/
                            try {
                                myCursor = mApplication.mContext.getContentResolver().query(
                                        Calls.CONTENT_URI, null,
                                        null, null, null);
                                if (myCursor == null || myCursor.getCount() == 0) {
                                    _id = myId + 1;
                                    Settings.System.putInt(mApplication.mContext.getContentResolver(),
                                            "location_id", _id);
                                } else {
                                    myCursor.moveToLast();
                                    _id = myCursor.getInt(myCursor.getColumnIndex("_id")) + 1;
                                    Settings.System.putInt(mApplication.mContext.getContentResolver(),
                                            "location_id", _id);
                                }
                            } finally {
                            ...
                            final String ipPrefixC = Settings.System.getString(
                                    mApplication.mContext.getContentResolver(), "c_dial");
                            String my_c_dial;
                            if (TextUtils.isEmpty(ipPrefixC)) {
                                my_c_dial = "17909";
                            } else {
                                my_c_dial =  ipPrefixC;
                            }

                            final String ipPrefixG = Settings.System.getString(
                                    mApplication.mContext.getContentResolver(), "g_dial");
                            String my_g_dial;
                            if (TextUtils.isEmpty(ipPrefixG)) {
                                my_g_dial = "17951";
                            } else {
                                my_g_dial =  ipPrefixG;
                            }
                            String myNumber = number;
                            /*End: Modified by bxinchun 2012-08-02, add try block to fix bug.*/

                            if (myNumber.startsWith(my_c_dial)) {
                                myNumber = myNumber.substring(my_c_dial.length());
                            } else if (myNumber.startsWith(my_g_dial)) {
                                myNumber = myNumber.substring(my_g_dial.length());
                            }
                            String myAddress = getAddress(myNumber);
                            ContentValues mContentValues = new ContentValues();
                            mContentValues.put("_id", _id);
                            mContentValues.put("number", number);
                            mContentValues.put("location", myAddress);
                            try{
                                mApplication.mContext.getContentResolver().insert(
                                        Uri.parse("content://com.khong.provider.phonemanager/address"),
                                        mContentValues);
    ...
    private String getAddress(String number) {
        ...
        /*Begin: Modified by bxinchun for possiable bug.*/
        Cursor cursor = null;
        try {
            cursor = mApplication.mContext.getContentResolver().query(uri, null, null, null, null);
        ...
        /*End: Modified by bxinchun for possiable bug.*/
        return result;
    }
    ...
    private void onCdmaCallWaitingReject() {
        ...
        if (ringingCall.getState() == Call.State.WAITING) {
            ...
            if (c != null) {
                ...
                /*ren_kang.hoperun 2012.7.12 change for bug of call time when call waiting*/
                if(CallTime.resetTime>0){
                duration = CallTime.resetTime * 1000;}
                Log.i(LOG_TAG, "rk_duration="+duration);
                ...
                CallLogAsync.AddCallArgs args =
                        new CallLogAsync.AddCallArgs(
                            mApplication.mContext, ci, logNumber, presentation,
                            callLogType, date, duration, c.getCall().getPhone().getSubscription());
